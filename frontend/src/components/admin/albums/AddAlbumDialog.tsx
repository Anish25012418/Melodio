...
import type {NewAlbum} from "@/types/album.ts";
...
const AddAlbumDialog = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      } else if (!newAlbum.title) {
        return toast.error("Please upload a title");
      } else if (!newAlbum.artist) {
        return toast.error("Please upload a artist");
      } else if (!newAlbum.releaseYear) {
        return toast.error("Please upload a releaseYear");
      }
      ...
      await axiosInstance.post(API_PATHS.ADMIN.CREATE_ALBUM, formData, {
        headers: {
  ...
  return (
    <Dialog open={albumDialogOpen} onOpenChange={setAlbumDialogOpen}>
      <DialogTrigger asChild>
        <Button className='bg-violet-500 hover:bg-violet-600 text-black hover:cursor-pointer'>
